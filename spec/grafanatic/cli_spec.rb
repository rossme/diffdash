...
RSpec.describe Grafanatic::CLI do
  let(:git_context) { instance_double(Grafanatic::GitContext) }

  before do
    allow(Grafanatic::GitContext).to receive(:new).and_return(git_context)
  ...
  describe "#execute" do
    ...
    context "with --dry-run flag" do
      it "does not attempt to upload" do
        ...
        expect(Grafanatic::GrafanaClient).not_to receive(:new)
