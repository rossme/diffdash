...
RSpec.describe Grafanatic::Config do
  ...
  describe "#dry_run?" do
    it "returns true when GRAFANATIC_DRY_RUN is 'true'" do
      allow(ENV).to receive(:[]).with("GRAFANATIC_DRY_RUN").and_return("true")
      expect(config.dry_run?).to be true
    end

    it "returns false when GRAFANATIC_DRY_RUN is not 'true'" do
      allow(ENV).to receive(:[]).with("GRAFANATIC_DRY_RUN").and_return("false")
      expect(config.dry_run?).to be false
    end

    it "returns false when GRAFANATIC_DRY_RUN is not set" do
      allow(ENV).to receive(:[]).with("GRAFANATIC_DRY_RUN").and_return(nil)
      expect(config.dry_run?).to be false
    end
  end
end
