...
RSpec.describe Grafanatic::Dashboard::Builder do
  let(:config) { Grafanatic::Config.new }

  def create_signal(type:, name:, metadata: {})
    Grafanatic::Signals::Signal.new(
      type: type,
      name: name,
  ...
  describe "#build" do
    context "with signals" do
      ...
      it "includes auto-generated message" do
        expect(result[:message]).to include("grafanatic")
      end
      ...
      describe "dashboard structure" do
        ...
        it "includes grafanatic tag" do
          expect(dashboard[:tags]).to include("grafanatic")
        end
